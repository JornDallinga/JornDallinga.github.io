...
We will start of with a false color image of our subset area previous defined by the extent function. We will simply use all bands available. In order to create a false color image, we will have to rerun our ProccesProbaVbatch without filtering the cloud/shadow pixels.
...
The significance parameter allows you to control the metrics p-values, and which to keep, within the harmonic model. Thus, if this parameter is set, metrics with a p-value > significance will be set to '0'. By doing so, we only bring metrics that differ significantly from each other into the next functions.
...
The following function runs the getHarmMetrics to our virtual raster brick and returns the model coefficients. Each coefficient is thus returned as a raster layer, where the output number of rasterlayers depend on the 'order' that you assigned. 
...
The getHarmMetricsSpatial is basically an 'all in one' function of the previous sections, applying the cloud filter to a region of interest using your selected band. Depending on your selected region of interest in the timeVrtProbaV2 function and your available cores/memory. The function also allows you to scale up your values, allowing you to switch from float to integers, reducing storage/memory use. This function still has some limitations, as the parameters 'robust' and 'n_years' might throw errors as these have not been efficiently tested yet. 

There are currently two functions within the ProbaV package that are able to stack all the requested time series from your selection. The 'timeVrtProbaV' and the 'timeStackProbaV'. The 'timeStackProbaV' will use the more conventional stacking from raster::stack, whereas the 'timeVrtProbaV' will use the gdalbuildvrt (from GDAL) function, to build a virtual raster stack. The 'timeVrtProbaV' requires you to set your extent within the function call (te parameter), whereas the 'timeStackProbaV' has to be cropped after stacking. 
...
```{r, echo= FALSE}
cat("System time of the Virtual raster stack: ", systime_vrt)

cat("System time of the Virtual raster stack: ", systime_stack)
```
...
So, time to grab something to eat or a coffee! Remember, this function still has some limitations, as the parameters 'robust' and 'n_years' might throw errors as these have not been efficiently tested yet. 
...
Once the function has successfully passed, we can see the output! The output is a brick of rasters that store the model coefficients (or metrics) applied in the getHarmMetricsSpatial function. Thus, the amount of bands returned (as raster layers) depends on the chosen 'order' in the getHarmMetricsSpatial. Due to the parallelization of the function, the names of the output (brick) get lost, and the chronological order with it. 

Since the getHarmMetricsSpatial function uses the getHarmMetrics function, but applies it to a raster, we can see the names of the metrics by evaluating our previously getHarmMetrics section. 
...
The tutorial and data availability is still under construction, so please report any errors or anomalies on the [github account](https://github.com/JornDallinga/JornDallinga.github.io) of this tutorial.

### Acknowledgements
The tutorial and related research have been done within the framework of the PROBA-V MEP project and with support and funding of the [ESA](https://proba-v-mep.esa.int/). 

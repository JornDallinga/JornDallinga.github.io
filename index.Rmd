...
b_vrt <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, return_raster = T, start_date = "2015-10-21", end_date = "2016-03-01")
...
b_vrt <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, return_raster = F, start_date = "2015-10-21", end_date = "2016-03-01")

dir.create('/userdata/cloud_filter')
out_name <- paste('/userdata/cloud_filter/cloud_filter.tif', sep = "")
```
```{r, eval=F}
# set variables
# threshold for blue
blue_c_filter <- mcCalc(x=b_vrt, fun=cloud_filter, minrows = 15, mc.cores = 20, logfile=logfile, out_name = out_name, overwrite = F, mc.preschedule = FALSE, thresholds = c(-80, Inf), cf_bands = 1, bands = 1)
```

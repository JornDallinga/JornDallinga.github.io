...
Below the data location of my PROABA_V data, and where I intend to store my new datasets (outdir)
...
The files should be stored in the folder structure used by vito, using YYYYMMDD notation for the folders that contain the .tif files. Where one folder per date (YYYYMMDD) contains files for all tiles. Through which folders you wish to loop the product information function, is up to you. For this tutorial, we created a preset of folder names for which we would like to receive the product information on the containing .tif files. If you wish to simply use one folder name, note that all the subfolders will be selected in the looping process, including folders that might contain incorrect/unprocessed .tif files.
...
df = list()
for (i in 1:length(dir_file)){
  dat <- getProbaVinfo(dir_file[i], pattern = patterns, tiles = c("X17Y06"))
...
```{r}
b_vrt <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = T, start_date = "2015-10-21", end_date = "2015-10-21")
  
df_probav_sm <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = F, start_date ="2015-10-21", end_date = "2015-10-21")
...
b_vrt_cloud <- timeVrtProbaV2(probav_cloud_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, return_raster = T, start_date = "2015-10-21", end_date = "2015-10-21", te = c(xmin,ymin,xmax,ymax))
...
# load the image in our virtual raster stack
b_vrt_cloud_mask <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, return_raster = T, start_date = "2015-10-21", end_date = "2015-10-21", te = c(xmin, ymin, xmax, ymax))
...
#b_vrt <- timeStackProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, #return_raster = T, start_date = "2015-10-21", end_date = "2016-03-01")
...
The cloud detection algorythm inbedded in the ProbaV data detects most of the clouds. Our additional cloud filter marks some additional clouds/shadow missed by the cloud detection algorythm. As can be observed, these missed cloud pixels are mainly distributed near the edges of clouds.
...
vrt_name <- file.path(paste0(outdir,"/",tiles, "_",paste0(bands_select, collapse = "_"), ".vrt"))
# Select all dates of the blue band
b_vrt <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, return_raster = T, start_date = "2014-01-21", end_date = "2016-09-01", te = c(xmin, ymin, xmax, ymax))

# Select random pixel of our raster stack
z <- zoo(c(b_vrt[500]), getZ(b_vrt))

#unique(df_probav_sm$band)
# read and assign the acquisition dates to a variable
df_probav_sm <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = F, start_date ="2014-01-21", end_date = "2016-09-01")
...
df_probav_sm <- timeVrtProbaV2(probav_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = F, start_date ="2015-10-21", end_date = "2016-03-01")
```

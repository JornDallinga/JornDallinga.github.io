---
title: "ProbaV"
author: "Jorn Dallinga, ProbaV package used from Johannes Eberenz"
date: "`r format(Sys.time(), '%d %B, %Y')`"
Version: "1.0 (updated 23-12)"
output:
...
#WUR ProbaV workflow ![WUR logo](http://www.wageningenur.nl/upload/f9a0b5f2-15c5-4b84-9e14-20ef02f5e265_wur-logo.png)
...
In order to preserve this observational continuity, Belgium decided to build a small satellite mission based on the successful ESA PROBA expertise, using state-of-the-art-technology. ProbaV (with "V" standing for Vegetation) was designed by a full Belgian consortium, fulfills all of the vegetation users specifications and is complement to the Sentinel-3 satellites to be launched after ProbaV.

ProbaV has a constellation of 3 cameras that daily observe the land surface and vegetation at similar spectral wavelengths (BLUE, RED, NIR, and SWIR) as SPOT-VGT, but with an improved spatial resolution (300 m and 100 m for the centre camera). These observations are processed into daily and 10-daily syntheses products,  available at 100 m, 300 m, and 1 km. The products can be downloaded from [vito](www.vito-eodata.be)'. [source](http://ProbaV.vgt.vito.be/) 

## Preperation

This script has been tested and runs on a LINUX Centos machine located on the Mission Exploitation Platform (MEP) from ESA, but might require some tweaking if used on other OS or versions. For more information on the MEP, please follow this [link](https://proba-v-mep.esa.int/node/12). Most functions are designed to run parallel on multi-core machines, such as on the MEP. This example script uses a small case study throughout the tutorial. However, when extending the functions to larger areas, system time could become significantly longer due to slower processing (depending on your computational power at your disposal).
...
The data used within these scripts can be downloaded from the [vito](http://ProbaV.vgt.vito.be/) website. Lets start with loading the required libraries and packages to illustrate a workflow on working with the ProbaV data. Additionally, we have to set the library path if this has not been set previously (This could depend on your MEP machine settings). When working from the MEP, (occasional) errors could occur with loading packages when the library path has not been added.

```{r, message=F, warning=F}
# add additional libary path
# Depends on which R version you work with
...
The ProbaV package used can be viewed [here](https://github.com/johanez/ProbaV). Many fixes, including spelling, and further updating of the package is still underway. Furthermore, we download (and source) some adjusted scripts directly using the following 'source_ttps' function from another github repository. 
...
Lets start with the preprocessing of the data. The following sections of the tutorial only work by following the ProbaV directory structure on the ProbaV MEP. The files should be stored in the folder structure used by vito (One folder per date, in YYYYMMDD format, which contains files for all tiles).

The ProbaV has 3 synthesis products, S1 (daily observations), S5 (mosaic-ed 5 day observation) and S10 (mosaic-ed 10 day observation). In the case of a 10-day synthesis product, the product has a minimum effect of cloud cover, resulting from selection of cloud-free acquisitions during the 10-day period. Additionally, their are two products among these daily products available, the Top of Atmosphere (TOA) and Top of Canopy (TOC). The TOC is the atmospheric corrected product, and in combination with the S5 synthesis, is the data we are going to use in this tutorial.

Below the data location of my ProbaV data (l0_dir), and where I intend to store my new data sets (outdir).
...
Throughout this tutorial, we will work with the tile 'X17Y06', which is located in Africa.

Lets map the extent of the tile:

```{r,eval=T, echo=F, warning=F, message=F}
tile <- 'X17Y06'
rast <- raster(paste0(getwd(), "/rsdata/ProbaV/results/pred_JD_full_", tile,  ".tif"))
library('rworldmap')
SPDF <- getMap()
mapCountryData(SPDF, nameColumnToPlot='continent', addLegend = F, mapRegion = 'Africa', mapTitle = 'Africa')
plot(extent(rast), add = T)
```
...
On the ProbaV-MEP VM, both NDVI and RADIOMETRY .tif files should be stored within the same folder. This can be observed by checking the folder location from which you read your ProbaV data (here 'l0_dir'). 

The radiometry brick contains 4 raster layers. Lets split the radiometry brick into single .tif layers (SWIR, NIR, RED, BLUE). You can select which tiles you would like to process. In this workflow, we are going to work with 3 bands, the Blue, SWIR and NDVI (NDVI is not part of the radiometry brick) bands. However, the function will extract all the bands that the radiometry brick contains, including the Red and NIR band. This allows you to play around with more band selections. 

NOTE: If you wish to work with different bands than within this tutorial, it might require some function tweaking. Feel free to adjust the functions accordingly on the github account (see bottom site). 
...
print(paste("We currently have ",nrow(df), " radiometry bands", sep = ""))
```
...
# Read a file name from your directory (l0_dir). We will use the tile 'X17Y06' at January 16th, 2016. 
...
# Read a file name from your output directory (outdir), following the same date and tile as above.
...
## Proccessing a ProbaV batch

The probaV::processProbaVbatch function uses the probaV::cleanProbaV function on a whole 'batch' of data. The starting date parameter helps selecting at which starting point you wish clean the data. The function uses parallel processing, which is useful when the machine has multiple processors or/and cores. The processing time of the following functions thus depend on the process capacity of your machine. In order to process both the radiometry and the NDVI data sets, we will have to run the probaV::processProbaVbatch function twice, with different patterns.
...
# similar for pattern = NDVI.tif$
processProbaVbatch(dir_file, 
                    pattern = patterns, tiles = tiles, start_date = "2015-10-21", end_date = NULL,
...
## Virtual Raster Stack

The following section sets the required parameters for the function 'virtual raster stack'. Most of the parameters are pretty basic, or are repeated here as reminder. 
...
Lets use the probaV::getProbaVinfo function again to visualize a certain tile data that we have.
...
The following parameters are required to be set. A small explanation is included on each parameter. 
...
# minrows is a parameter for the coming raster processing functions. It allows you to set the minimal raster rows that the coming calc functions will apply their algorithm on simultaneously. The minrows parameter depends on the computational capacity of your machine. Currently, there is no optimal setting for the amount of minrows and cores. However, it is generally adviced that with large datasets, the minimal rows parameter should be set lower (e.g 7) to avoid crashes.
minrows = 15

# check for available cores and set cores (minus 1). The amount of cores also depends on your available RAM. If you have very little RAM but many cores, the cores might run out of memory and crash. For a machine with 32GB of RAM, around 10-15 cores could be used, but also depends if more users are working with your machine at the same time. The optimal amount of 'minimal rows' and amount of cores might require tweaking.
...
# set logfile name. This will save a log file which allows you to trace back any warnings or errors that originate from your functions. 
...
# set raster options. This is optional and feel free to tweak these settings.
...
Select bands:
Band selection for the the stacking of raster layers. For this tutorial, we use the BlUE, SWIR and NDVI bands for the classification. However, you can select any of the available bands from the ProbaV sensor (e.g. including NIR/RED). 

```{r}
# Select the bands to use in sequential functions
bands_select <- '(NDVI|SWIR|BLUE|RED0)' # e.g. '(BLUE|SWIR|NDVI)' or '(BLUE|SWIR)' or 'NDVI'

# Include the '_sm.tif' extension to match file names as a result of the 'ProcessProbaVBatch' 
bands_sel <- paste(bands_select,'_sm.tif$', sep = "")
```


Create a virtual stack from selected bands. The virtual raster stacking is an alternative to the more conventional raster layer stacking through [stack](https://www.rdocumentation.org/packages/raster/versions/2.5-8/topics/stack) or [brick](https://www.rdocumentation.org/packages/raster/versions/2.5-8/topics/brick). Virtual raster stacking is an R wrapper for the 'gdalbuildvrt' function that is part of the Geospatial Data Abstraction Library (GDAL). Opposed to raster::stack or raster::brick, the virtual rasterstack function creates a file (.vrt), linked to the filepath environment of selected data sets. Major benefit is that 'GDAL vrt' was found to query faster than raster::stack.
...
# Returns a dataframe (return_raster = F)  
...
Lets see what our dataframe contains. The output of the virtual raster stack which returns as a dataframe is a collection of relevant product information within the limits of your date and tile selection.

```{r}
# returning the first four rows of the created dataframe.
...
"Clouds in satellite observations obstruct the retrieval of vegetation parameters. Therefore a proper cloud screening is pivotal in the pre-processing of the various value-added products. The ProbaV cloud detection algorithm is a modified version of the method applied to the SPOTVGT
...
and SWIR bands is no longer valid for ProbaV, due to the observation time difference."  [source](http://ProbaV.vgt.vito.be/sites/default/files/Product_User_Manual.pdf) p.17. 

In addition to the added cloud/shadow algorithm applied on the ProbaV products, the ProbaV R package adds an additional cloud filter which requires some manual interpretation. The following section will visualize the difference between the cloud/shadow algorithm from ProbaV and the created cloud filter function in the ProbaV R package.

We will start of with a false color image of our subset area previous defined by the extent function. We will simply use all bands available. In order to create a false color image, we will have to rerun our probaV::ProccesProbaVbatch without filtering the cloud/shadow pixels.
...
# select directory with clouds (cloud_dir)
ProbaV_cloud_dir <- paste(getwd(),'/',cloud_dir,'/', sep = "")
...
# Adding the parameter 'te' allows you to crop the selected images to a user defined extent.
...
Well that looks cloudy! Good, now we can start adding the ProbaV cloud mask and the ProbaV R package cloud filter. Lets start with visualizing the cloud detection method in-bedded in the ProbaV data.
...
# subset a single raster band, I will take the BLUE band.
b_vrt_cloud_mask <- subset(b_vrt_cloud_mask,1)
```


Lets plot the original cloud detection method in-bedded within the ProbaV data.
...
Looks good! The orange area here visualizes the clouds/shadow and bad pixels detected by the cloud detection algorithm within ProbaV. If you look closely, you can still observe cloud and shadow remnants, mostly near the orange masked clouds. Although this original cloud detection algorithm already masked out almost all clouds, we would like to clear as many clouds (and shadow) as possible, so we have a clear reflectance of the pixels. Because remaining clouds in remote sensing time-series can impair the extraction of seasonal information. 
...
The 'cloud\_filter' function is an intensive computational and disk volume demanding function, as it returns all the .tif files (as brick) within you 'date' selection. However, since we select a subset by setting an extent in the 'probaV::timeVrtProbaV' function, the output files should not be that big in volume. The 'cloud_filter' is not part of the 'ProbaV' package, but is derived from it. The threshold parameters are similar to existing functions within the 'ProbaV' package. Where:

  - x = Virtual stack of .tif files
  - ProbaV\_sm_dir: Directory containing the processed ProbaV bands as a result of the 'ProcessProbaVBatch'
  ...
  - ... e.g. overwrite (Default = False)
  
Minrows and mc.cores should be considered carefully when running the cloud_filter on whole tiles. The settings depend on the available VM specs, such as cores and RAM. We have not yet found a way to deal with this issue effectively. Hence, if you run into errors, try to tune down the 'minrows' or the 'mc.cores' (e.g. minrows = 2 when working on whole tiles).
...
Lets select the same *date* as our false color image from the raster brick we just created. In this case, the first raster subset of our filter.
...
The 'cloud_filter' function returns the Quality Control values. Meaning: "QC" is the status of observations: 0=Missing/band input QC, 1=good, 2=temporal outlier. Here we assign new values to our raster to visualize what the 'SmoothLoes' function detects on clouds/shadow
...
Alright! Now lets plot our cloud filter!
...
Lets plots the original false color again against the 2 cloud filters
...
The cloud detection algorithm in-bedded in the ProbaV data detects most of the clouds. The additional 'cloud_filter' marks some clouds/shadow fragments missed by the cloud detection algorithm. As can be observed, these missed cloud pixels are mainly distributed near the edges of existing clouds.
...
# Select all dates of the blue band and return it as a raster.
...
set.seed(42)
z <- zoo(c(b_vrt[runif(1, 0, ncell(b_vrt))]), as.Date(getZ(b_vrt), "%Y%j"))

# read and assign the acquisition dates to a variable (df_ProbaV_sm). Notice that the return_raster variable is set to 'False', and will thus return a dataframe with metadata on the selected dates.
df_ProbaV_sm <- timeVrtProbaV(ProbaV_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = F, start_date ="2014-01-21", end_date = "2016-09-01", te = c(xmin, ymin, xmax, ymax))
...
Observations where difference to the LOESS model exceeds a threshold are masked as clouds or cloud shadow. The 'ProbaV::smoothloess' function applies a smoothing approach, by fitting a locally weighted scatterplot smoothing model. We used this function before in the cloudfilter section of this tutorial. The span parameter controls the degree of smoothing. The higher the span value, the higher the smoothing of a time series object. 
...
As can be observed above, smoothing the model by setting a (too) high span value can smooth out vegetation trends which might be important for your classification later on. Altough we plot the 'filled' values here as well, they are not used in the final models. Filling gaps between time observations based on linear models can create a false interpretation of vegetation trends, especially when the time between observations is large (e.g. months). 

## Harmonic metrics

We use a seasonal model approach and descriptive statistics. Per band and pixel i we derive median and percentiles and fit a linear harmonic model to the time series. The coefficients of this linear model are then returned as metrics (such as min,max values) and seasonality (cos, sin, trend). Here the dates represent the frequency within the harmonic seasonal model. The Cosine (cos) and Sine (sin) reflect the amplitude and phase shift, respectively, but cannot be interpreted as such directly from the cos and sin.

Second or Third order harmonics will create an additional output of the Cosine and Sine multiplied the selected order. Higher order harmonics will be added to the lower order harmonics.

----------------------------------------------------------------------------------------

Lets show the formula used to calculate the 'cos' and 'sin' values. 
  ...
  **formula <- x ~ (co + si) # Here x is an observation through time**
  
  with higher order harmonics, the formula is adjusted by multiplying the cos and sin with the order:
  ...
  These additional coefficients (co2 and si2) will be added to the formula:
  
  **formula <- x ~ co + si + co2 + si2**
  
----------------------------------------------------------------------------------------

The probaV package currently supports the required 'harmonic order' of atleast 1 to up to 3. 

QC-good parameter ensures that reflectance values that are missing (0 or not 1) are omitted (the fitted values are thus **not** used). Reflectance values that are marked as clouds/shadow by the 'probaV::smoothLoess' function (value 2 in QC-good) will be ignored by the linear model.

Lets test the probaV::getHarmMetrics function on our timeseries(z).

```{r,eval=T}
# Run the smoothloess function on timeseries (z)
f <- smoothLoess(tsx = z, QC_good=NULL, dates=dates , res_type=c("all"), span=0.3)

# With the return_model = T, we return the harmonic model. Otherwise, it will return the coefficients (metrics)
model_metrics <- getHarmMetrics(f$x,QC_good = f$QC_good ,dates = dates, order = 1, return_model = T)
...
```{r, eval=T}
# Run the getHarmMetrics function with return_model = F, so we return just the metrics
model_metrics <- getHarmMetrics(f$x,QC_good = f$QC_good ,dates = dates, order = 1, return_model = F)
```
...
# Order 2
model_metrics <- getHarmMetrics(f$x,QC_good = f$QC_good ,dates = dates, order = 2)
...
The following function runs the probaV::getHarmMetrics to our virtual raster brick and returns the model coefficients. Each coefficient is thus returned as a raster layer, where the output number of rasterlayers depend on the 'order' that you assigned. 

Lets start of with setting the required variables for the function parameters. 

```{r, eval=F}
# create output name on the metrics
out_name <- file.path(getwd(), paste0("rsdata/ProbaV/metrics/",tiles[1],"_harm_lm2_loess_03_scaled.envi"))
...
# Create dataframe for metadata (notice the return_raster variable is set to 'False'. Thus we get a dataframe in return)
df_ProbaV_sm <- timeVrtProbaV(ProbaV_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = F, start_date ="2014-01-21", end_date = "2016-09-01", te = c(xmin, ymin, xmax, ymax))
...
Check layers, bands, blocks and cores! Each block represents the number of rows that the 'probaV::getHarmMetricsSpatial' function applies to. This speeds up the calculation time significantly. As normally the linear model of getHarmMetrics would be calculated to each cell of a rasterbrick, it now applies the function simultaneously to 15 (or n minrows) rows in parallel. 
...
Now lets try out the probaV::getHarmMetricsSpatial function! Most of the functions parameters must be familiar by now. The cf-bands (cloud_free bands) is the coerced selection of the bands to which you would like to apply the cloud filter. My 'bands-select' variable is set to 3 bands, the BLUE, SWIR and NDVI. By setting the cf-bands to c(1,2), I apply the additional cloud filter to band 1 and 2 from my selected bands, in this scenario the BLUE and SWIR. The thresholds follow a similar process, where the given thresholds below apply to the first 2 bands. Each 2 values represent the min and max of your selected threshold, which are the min/max values to detect additional clouds/shadow. 

The probaV::getHarmMetricsSpatial is basically an 'all in one' function of the previous sections, applying the cloud filter to a region of interest using your selected band. The computation time depends on your selected region of interest and your available cores/memory. The function also allows you to scale up your values, allowing you to switch from float to integers, reducing storage/memory use. This function still has some limitations, as the parameters 'robust' and 'n_years' might throw errors as these have not been efficiently tested yet. 

There are currently two functions within the ProbaV package that are able to stack all the requested time series from your selection. The 'probaV::timeVrtProbaV' and the 'probaV::timeStackProbaV'. The 'probaV::timeStackProbaV' will use the more conventional stacking from raster::stack, whereas the 'probaV::timeVrtProbaV' will use the gdalbuildvrt (from GDAL) function, to build a virtual raster stack. The 'probaV::timeVrtProbaV' requires you to set your extent within the function call ('te' parameter), whereas the 'probaV::timeStackProbaV' has to be **cropped** after stacking. 
...
# Create virtual stack (using probaV::timeVrtProbaV)
b_vrt <- timeVrtProbaV(ProbaV_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles, return_raster = T, start_date = "2014-01-21", end_date = "2016-09-01", te = c(xmin, ymin, xmax, ymax))

# Create a raster brick using the raster::stack function
...
# Return dataframe of our selected metadata.
df_ProbaV_sm <- timeVrtProbaV(ProbaV_sm_dir, pattern = bands_sel, vrt_name = vrt_name, tile = tiles[1], return_raster = F, start_date ="2014-01-21", end_date = "2016-09-01", te = c(xmin, ymin, xmax, ymax))
```
...
Lets see if there is a difference in time! Consider that this is a small subset, but the difference in time on the machine is significant on larger data sets.

```{r, echo= F, hold = T}
cat("total elapsed time of the Virtual raster stack: ", systime_vrt[3])

cat("total elapsed time of the raster::stack: ", systime_stack[3])
```
...
So, time to grab something to eat or a coffee! Remember, this function still has some limitations, as the parameters 'robust' and 'n_years' might throw errors as these have not been efficiently tested yet. If the function throws additional errors, be sure to try and tweak down the minrows and mc.cores.
...
Once the function has successfully passed, we can see the output! The output is a brick of rasters that store the model coefficients (or metrics) applied in the probaV::getHarmMetricsSpatial function. Thus, the amount of bands returned (as raster layers) depends on the chosen 'order' in the probaV::getHarmMetricsSpatial. Due to the parallelization of the function, the names of the output (brick) get lost, and the chronological order along with it. 

Since the probaV::getHarmMetricsSpatial function uses the probaV::getHarmMetrics function, but applies it to a raster, we can see the names of the metrics by evaluating our previously probaV::getHarmMetrics section. 

Lets plot the first 6 layers from the created metrics brick. As can be observed, the names are lost of the bands, and the raster values have been scaled by the selected 'scale_f' parameter in the probaV::getHarmMetricsSpatial function. 
...
```
```{r, eval=F, echo=F}
out_name <- file.path("/userdata/metricsJD", paste0(tiles,"_harm_lm2_loess_03_scaled.envi"))
b_metrics <- brick(out_name)
```
...
The following block runs the ranger function on our training data (df\\_model_JD) and writes the output ranger model to a RDS file.
...
Now the final step of this tutorial! Here we run the probaV::mcPredictSpatial function that classifies our metrics based on the ranger model we created. We read back our created ranger model from the directory. 
...
# set the output filename of the prediction results
out_name <- paste0(getwd(), "/rsdata/ProbaV/results/pred_JD_")
...
for (tile in tiles){
  ...
  out_name <- paste0(out_name, tile,  ".tif")
...
NOTE: Be sure to add and/or change the colors based on the amount of classes you created! 
...
# add/change colours if you have more legend items. The colours should be defined in the order as given by the 'rat' variable. 
rat # # The first row of 'rat' (bare) equals the first colour below (chocolate4).
my_col = c('chocolate4','darkgreen','yellowgreen')
...
Below the our tile with the reference points, followed by a classified image. 
...
Congrats! Now have fun applying this tutorial on your region of interest! Keep in mind that there are several limitations on the cloud filter section. The discussed approach is less suited to distinguish ice clouds from snow/ice! 

This tutorial has been tested on the S5 (5-day) 100m resolution product of ProbaV. It might return unexpected results or errors if attempted on different data sets, and has not been tested yet on other sensors, and products of ProbaV.

The tutorial and data availability is still under construction, so please report any errors or anomalies on the [github account](https://github.com/JornDallinga/JornDallinga.github.io) of this tutorial or directly on the [probaV package](https://github.com/johanez/probaV).

### Acknowledgements
The tutorial and related research activities have been done within the framework of the PROBA-V MEP project and with support and funding of the [ESA](https://proba-v-mep.esa.int/).

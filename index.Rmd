...
The 'cloud_filter' function is an intensive computational and disk volume demanding function, as it returns all the .tif files (as brick) within you date selection. However, since we select a subset by setting an extent in the 'timeVrtProbaV' function, the output files should not be that big in volume. The 'cloud_filter' is not part of the 'probaV' package, but is derived from it. The threshold parameters are similar to existing functions within the 'probaV' package. Where:
  ...
  - Thresholds: The distance values from the smoothloess model to mask as clouds/shadow
...
We use a seasonal model approach and descriptive statistics. Per band and pixel i we derive median and percentiles and fit a liner harmonic model to the time series. The coefficients of this linear model are then used as metrics for the descriptive statistics (such as min max values) and seasonality (cos, sin). 

The number of output coefficients (metrics) depend on the parameter 'order'. QC-good parameter ensures that values that are missing (0 or not 1) are omitted. Also values that are marked as additional clouds by the smoothLoess function (value 2 in QC-good) will thus be ignored by the linear model.


```{r,eval=T}
model_metrics <- getHarmMetrics(f$x,QC_good = f$QC_good ,dates = dates, sig = .95, order = 1)
model_metrics
```

Lets see the difference in output from different 'order' levels. If you select a higher  
...
And the full tile we have been processing in the tutorial. Below the tif. tile with the reference points, and the classified image. 
...
Congrats! Now have fun applying this tutorial on your region of interest! Keep in mind that there are several limitations on the cloud filter section. Specially to areas that occasionally receive snow! 
